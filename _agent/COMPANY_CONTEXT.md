# VCT PLATFORM — Company Context

> **Mục đích**: Tài liệu nền tảng mà MỌI vai trò trong tổ chức PHẢI đọc trước khi thực thi bất kỳ tác vụ nào.
> Cập nhật file này khi có thay đổi lớn về sản phẩm, thị trường, hoặc chiến lược.

---

## 1. Về Công ty

| Thông tin     | Chi tiết                          |
| ------------- | --------------------------------- |
| **Tên**       | Công Ty TNHH VCT Platform         |
| **Loại hình** | Công ty Công nghệ — SaaS Platform |
| **Lĩnh vực**  | [Cập nhật: Phần mềm]              |
| **Thành lập** | [Cập nhật: 09/03/2026]            |
| **Quy mô**    | [Cập nhật: 01 người]              |
| **Trụ sở**    | Việt Nam                          |
| **Website**   | [Cập nhật: vctplatform.vn]        |
| **GitHub**    | https://github.com/VCT-PLATFORM   |

## 2. Tầm nhìn & Sứ mệnh

### Tầm nhìn (Vision)

> [Cập nhật: Tầm nhìn dài hạn 5-10 năm của VCT Platform]

### Sứ mệnh (Mission)

> [Cập nhật: Sứ mệnh — VCT Platform tồn tại để giải quyết vấn đề gì cho ai?]

### Giá trị Cốt lõi (Core Values)

1. [Cập nhật: VD: Innovation — Luôn tìm cách tốt hơn]
2. [Cập nhật: VD: Customer-First — Khách hàng là trung tâm]
3. [Cập nhật: VD: Transparency — Minh bạch trong mọi việc]
4. [Cập nhật: VD: Excellence — Không chấp nhận tầm thường]
5. [Cập nhật: VD: Ownership — Mỗi người là chủ của kết quả]

## 3. Sản phẩm & Dịch vụ

### Sản phẩm Chính

| Sản phẩm   | Mô tả        | Khách hàng Mục tiêu | Giá             |
| ---------- | ------------ | ------------------- | --------------- |
| [Tên SP 1] | [Mô tả ngắn] | [Segment]           | [Pricing model] |
| [Tên SP 2] | [Mô tả ngắn] | [Segment]           | [Pricing model] |

### Unique Value Proposition (UVP)

> [Cập nhật: Điều gì làm VCT Platform KHÁC BIỆT so với đối thủ? 1-2 câu]

### Product-Market Fit Signals

- [Cập nhật: Retention rate, NPS, growth rate, hoặc bất kỳ signal nào]

## 4. Khách hàng Mục tiêu

### Ideal Customer Profile (ICP)

| Attribute          | Detail                                   |
| ------------------ | ---------------------------------------- |
| **Ngành**          | [Cập nhật]                               |
| **Quy mô**         | [Cập nhật: Số NV, doanh thu]             |
| **Vị trí địa lý**  | [Cập nhật]                               |
| **Pain Points**    | [Cập nhật: 3-5 vấn đề chính họ gặp]      |
| **Decision Maker** | [Cập nhật: Chức vụ người quyết định mua] |
| **Budget**         | [Cập nhật: Ngân sách điển hình]          |

### User Personas

> [Cập nhật: Mô tả 2-3 personas chính sử dụng sản phẩm]

## 5. Cạnh tranh

### Competitive Landscape

| Đối thủ     | Mạnh ở      | Yếu ở      | VCT khác biệt |
| ----------- | ----------- | ---------- | ------------- |
| [Đối thủ 1] | [Điểm mạnh] | [Điểm yếu] | [So sánh]     |
| [Đối thủ 2] | [Điểm mạnh] | [Điểm yếu] | [So sánh]     |
| [Đối thủ 3] | [Điểm mạnh] | [Điểm yếu] | [So sánh]     |

### Competitive Moat (Lợi thế cạnh tranh bền vững)

> [Cập nhật: Network effects? Data? Tech? Brand? Switching cost?]

## 6. Technology Stack

| Layer          | Technology                              |
| -------------- | --------------------------------------- |
| **Frontend**   | Next.js, React, TypeScript, TailwindCSS |
| **Backend**    | NestJS, TypeScript                      |
| **Database**   | PostgreSQL (Supabase)                   |
| **Messaging**  | NATS JetStream                          |
| **Hosting**    | [Cập nhật: Vercel, AWS, GCP?]           |
| **CI/CD**      | GitHub Actions                          |
| **Monitoring** | [Cập nhật]                              |
| **Analytics**  | [Cập nhật]                              |

## 7. Metrics Hiện tại (KPIs)

> ⚠️ **Cập nhật hàng tháng** để agent luôn có data mới nhất khi phân tích.

| Metric            | Giá trị Hiện tại | Mục tiêu Q tiếp theo |
| ----------------- | ---------------- | -------------------- |
| MRR / Revenue     | [Cập nhật]       | [Cập nhật]           |
| Users / Customers | [Cập nhật]       | [Cập nhật]           |
| Churn Rate        | [Cập nhật]       | [Cập nhật]           |
| NPS               | [Cập nhật]       | [Cập nhật]           |
| Team Size         | [Cập nhật]       | [Cập nhật]           |
| Runway            | [Cập nhật]       | [Cập nhật]           |

## 8. OKRs Hiện tại

### Company OKRs — [Q hiện tại]

> [Cập nhật mỗi quý]

**Objective 1**: [Cập nhật]

- KR1: [Cập nhật]
- KR2: [Cập nhật]
- KR3: [Cập nhật]

**Objective 2**: [Cập nhật]

- KR1: [Cập nhật]
- KR2: [Cập nhật]
- KR3: [Cập nhật]

---

> 📝 **Hướng dẫn**: CEO hoặc người dùng cập nhật file này khi có thay đổi. Mọi thay đổi sẽ tự động ảnh hưởng đến cách toàn bộ tổ chức AI vận hành.
