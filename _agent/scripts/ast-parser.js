...
if (process.argv.length < 3) {
  console.error('Usage: node ast-parser.js <path/to/file.js>');
  process.exit(1);
}
...
for (let i = 0; i < lines.length; i++) {
  ...
  } else if (methodMatch && !line.includes('if') && !line.includes('for') && !line.includes('while') && !line.includes('switch') && !line.includes('catch')) {
    // Basic heuristic to catch Class Methods.
    result.functions.push({ name: methodMatch[1], type: 'Method/Other', lineStart: i + 1 });
  }
}

// Print compact JSON output so LLM can read 10 lines instead of 1000 lines.
